...
import { Uint8, Uint16, Uint32, SerializableNumber } from '../../src/index';
...
class ExampleClass implements ISerializable {
  ...
  constructor(byte: number = 0, ushort: number = 0, uint: number = 0) {
    this.testByte = byte;
...
describe('Serializable class with primitives only', () => {
  it('object should serialize into Uint8Array', () => {
    const obj = new ExampleClass(42, 2048, 131_072);
    const expectedBytes = new Uint8Array([0x2a, 0x00, 0x08, 0x00, 0x00, 0x02, 0x00]);
  ...
  it('deserialize should return object', () => {
    const bytes = new Uint8Array([0x81, 0x88, 0x27, 0xb3, 0xa7, 0x2f, 0x00]);
    const expectedObj = new ExampleClass(129, 10_120, 3_123_123);
